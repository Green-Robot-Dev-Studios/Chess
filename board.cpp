...
void Board::resetBoard() {
    ...
    for (int r = 0; r < boardSize; ++r) {
        ...
        for (int c = 0; c < boardSize; ++c) {
            board[r].push_back(Square(r, c)); // Create Square at position (r, c)
        }
    }

    // Place Pawns
    for (int c = 0; c < boardSize; ++c) {
        board[1][c].setPiece(std::make_shared<Pawn>(Black, 1, c)); // Black pawns on the 2nd row
        board[6][c].setPiece(std::make_shared<Pawn>(White, 6, c)); // White pawns on the 7th row
    }
    ...
    // Place Knights
    board[0][1].setPiece(std::make_shared<Knight>(Black, 0, 1));  // Black Knights
    board[0][6].setPiece(std::make_shared<Knight>(Black, 0, 6));
    board[7][1].setPiece(std::make_shared<Knight>(White, 7, 1));  // White Knights
    board[7][6].setPiece(std::make_shared<Knight>(White, 7, 6));

    // Place Bishops
    board[0][2].setPiece(std::make_shared<Bishop>(Black, 0, 2));  // Black Bishops
    board[0][5].setPiece(std::make_shared<Bishop>(Black, 0, 5));
    board[7][2].setPiece(std::make_shared<Bishop>(White, 7, 2));  // White Bishops
...
std::vector<std::vector<Square>> Board::getState() {
    return board;
}
